...
from app.agents.writing_graph import build_writing_graph
...
class WritingService:
    ...
    def generate_section(self, request: WritingRequest) -> WritingResponse | AgentRunAcceptedResponse:
        ...
        topic = request.topic or project.title
        if request.mode == "async":
            run = AgentRunner(self.db).create_run(str(project_id), "writing", request.section_title, status_value="queued")
            payload = request.model_dump()
            payload.update({"mode": "sync", "run_id": str(run.id)})
            from app.workers.tasks_writing import generate_section_draft_task

            get_document_queue().enqueue(generate_section_draft_task, payload, job_timeout="30m")
        ...
        final_state, _ = AgentRunner(self.db).run_sync(
            ...
            intent="writing",
            user_query=request.section_title,
        ...
        if output is None:
            raise AppError("Writing output was not saved", status.HTTP_500_INTERNAL_SERVER_ERROR)

        return WritingResponse(
            output_id=str(output.id),
            section_type=request.section_type,
            section_title=request.section_title,
            section_markdown=output.content_markdown or "",
            section_json=output.content_json or {},
            citations=output.citations or [],
            warnings=(output.content_json or {}).get("warnings", []),
            created_at=output.created_at,
        )

    def list_drafts(self, project_id: str) -> list[GeneratedOutput]:
        ...
        return list(
            self.db.execute(
                ...
                .where(GeneratedOutput.project_id == project_uuid, GeneratedOutput.output_type == "section_draft")
    ...
    def get_draft(self, output_id: str) -> GeneratedOutput:
        output = self.db.get(GeneratedOutput, self._parse_uuid(output_id, "output_id"))
        if output is None or output.output_type != "section_draft":
            raise AppError("Draft not found", status.HTTP_404_NOT_FOUND)
        return output
